	...
	while (!file.eof() && file.good())
	{
		...
		case 7:
			...
			if (str != NULL && strncmp(str, "OFFSET", strlen("OFFSET")) == 0)
			{
				...
				curLink = new Link(); //creates new link for end site
				Link* top = stack.top(); //obtain parent link information
				newSkel->AddToSkeleton(curLink, top->GetName()); //link the current end site to the parent
				//chose unknown joint type since 0 DOF and different types of end sites present
				curLink->SetJointType("unknown"); //Added case handling in SetSkelState for J_UNDEF
				curLink->SetParTranslation(x, y, z); //add offsets of the end site to m_parTrans variables
