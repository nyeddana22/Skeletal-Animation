...
//Sets the local transformationCD  matrix of the link and location of the joint
//based upon the state data
void Link::UpdateAndRecurse(Skeleton* pSkel)
{	
	//parent transformation matrix
	mat4x4 parentTrans;
	//openGL function to create matrix for current link transformations
	glMatrixMode( GL_MODELVIEW );
	//openGL function to make the current matrix an identity matrix
	glLoadIdentity();
	//creates array to store 16 float values
	GLfloat temp[16];
	//current transformation matrix
	mat4x4 transMat;
	//if current link is the root, only translation is applied
	if(m_parNde == NULL) {
		//openGL function to make a translation matrix with x,y,z offsets
		glTranslatef(m_parTrans[0], m_parTrans[1], m_parTrans[2]);
		//obtains the translation matrix and copies values into temp
		glGetFloatv(GL_MODELVIEW_MATRIX, temp);
		//converts temp to a matrix by storing in world transformation matrix
		memcpy(m_LToWTrans, temp, sizeof(float) * 16);
	}
	//if it is a child node, first translation and then rotation is applied
	else {
		//translation
		glTranslatef(m_parTrans[0], m_parTrans[1], m_parTrans[2]);
		glGetFloatv(GL_MODELVIEW_MATRIX, temp);
		memcpy(transMat, temp, sizeof(float) * 16);
		//obtain parent transformation matrix and store in parentTrans
		memcpy(parentTrans, m_parNde->m_LToWTrans, sizeof(float) * 16);
		//generate rotation matrix
		MakeLinkRotMatrixLocal(transMat);
		//combine the current transformation with the parent's transformation and store
		mat4x4_mul(m_LToWTrans, parentTrans, transMat);
	}

	//Recurse this function for all children nodes if they exist
...
void Link::CalcVertexLocations(int maxEntries, int* curLocation, VERTEX** outCoords)
{
	...
	//If the current link is the root, there is no parent node
	if(m_parNde == NULL) {
		GetLToWTransMat(tm);
	}
	//If it is a child node, we call the parent's transformation matrix because the first 12 vertices are not included in the calc
...
void Link::MakePyramid(float axis[3], VERTEX** outCoords)
{
	...
	float offset = len / 20.0; //changes thickness of the triangle
	int startInd = 0;
	for (int i = 0; i < 4; i++) //defines length of the pyramids (height)
	{
		out[startInd].y = 0;
		out[startInd + 2].y = 0;
		out[startInd + 1].x = 0;
...
void Link::TransformVERTEX(mat4x4 tm, VERTEX* vertIn, VERTEX* vertOut)
{
	...
	//column major transformtion to multiply translations with original vertices to obtain transformed vertices
	vec4 in, out;
	in[0] = vertIn->x;
	in[1] = vertIn->y;
	in[2] = vertIn->z;
...
void Link::MakeLinkRotMatrixLocal(mat4x4 rot)
{
	...
	//for each rotation channel of the current joint, generate each x,y or z rotation and multiply each
	//to generate a final rotation matrix 
	for(int i=0; i<m_jointTypeToNumRotations[m_jointType]; i++) {
		
		//if X rotation
		if (m_axisOrder[i] == 0) { 
			mat4x4_rotate(rot, rot, 1, 0, 0, m_dofValues[i]);
		}
		//if Y rotation
		else if (m_axisOrder[i] == 1) {
			mat4x4_rotate(rot, rot, 0, 1, 0, m_dofValues[i]);
		}
		//if Z rotation
		else if (m_axisOrder[i] == 2) {
			mat4x4_rotate(rot, rot, 0, 0, 1, m_dofValues[i]);
		}
	}
}
